...
def main():
    ...
    max_concurrent = 50  # Maximum number of concurrent jobs
    ...
    for trial, params in enumerate(all_params):
        ...
        result = subprocess.run(['qsub', '-pe', 'smp', '8', script_path], capture_output=True, text=True)
        if result.returncode == 0:
